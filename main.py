...
if __name__ == "__main__":
    ...
    weights = load_weights_from_file('weights/90sec_3pos_dt200_simplecost/gen20_cost-41615_avg-41615')
    ...
    def costfunc(gene):
        ...
        simulation_time = 90
        delta_t = 200
        cost -= environment.simulate(graphics, time_dilation, simulation_time, weights=gene, static_delta_t=delta_t, start_x=400, start_y=175, start_angle=0)  # Start in the middle
        cost -= environment.simulate(graphics, time_dilation, simulation_time, weights=gene, static_delta_t=delta_t, start_x=80, start_y=80, start_angle=45)  # Start in corner
        cost -= environment.simulate(graphics, time_dilation, simulation_time, weights=gene, static_delta_t=delta_t, start_x=80, start_y=400, start_angle=90)  # Start next to a wall
        return cost
